...
u32 grindstm_state::screen_update_toaplan2(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
...
void grindstm_state::screen_vblank(int state)
{
	// rising edge
	if (state)
	{
...
void grindstm_state::vfive(machine_config &config)
{
	...
	m_screen->set_screen_update(FUNC(grindstm_state::screen_update_toaplan2));
	m_screen->screen_vblank().set(FUNC(grindstm_state::screen_vblank));
	...
	MCFG_VIDEO_START_OVERRIDE(grindstm_state,toaplan2)
