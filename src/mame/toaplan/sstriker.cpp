...
class sstriker_state : public driver_device
{
public:
...
TILE_GET_INFO_MEMBER(sstriker_state::get_text_tile_info)
{
...
void sstriker_state::create_tx_tilemap(int dx, int dx_flipped)
{
	m_tx_tilemap = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(*this, FUNC(sstriker_state::get_text_tile_info)), TILEMAP_SCAN_ROWS, 8, 8, 64, 32);
...
void sstriker_state::coin_w(u8 data)
{
	/* +----------------+------ Bits 7-5 not used ------+--------------+ */
...
void sstriker_state::mahoudai_68k_mem(address_map &map)
{
	...
	map(0x21c03c, 0x21c03d).r(FUNC(sstriker_state::video_count_r));
