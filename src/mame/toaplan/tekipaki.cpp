...
class toaplan2_tekipaki_state : public driver_device
{
public:
	toaplan2_tekipaki_state(const machine_config &mconfig, device_type type, const char *tag)
...
void toaplan2_tekipaki_state::toaplan2_reset(int state)
{
	if (m_audiocpu != nullptr)
		m_audiocpu->pulse_input_line(INPUT_LINE_RESET, attotime::zero);
}

void toaplan2_tekipaki_state::coin_w(u8 data) // MOVE TO DEVICE!
{
	/* +----------------+------ Bits 7-5 not used ------+--------------+ */
...
VIDEO_START_MEMBER(toaplan2_tekipaki_state,toaplan2)
{
...
u32 toaplan2_tekipaki_state::screen_update_toaplan2(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
...
void toaplan2_tekipaki_state::screen_vblank(int state)
{
	// rising edge
	if (state)
	{
...
//  PORT_CONFSETTING(        0x000e, DEF_STR( Japan ) )
	...
	PORT_BIT( 0x0010, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_READ_LINE_MEMBER(FUNC(toaplan2_tekipaki_state::c2map_r))
INPUT_PORTS_END
...
static INPUT_PORTS_START( whoopee )
	...
	PORT_MODIFY("JMPR")
	PORT_BIT( 0x0010, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_READ_LINE_MEMBER(FUNC(toaplan2_tekipaki_state::c2map_r))   // bit 0x10 sound ready
INPUT_PORTS_END
...
int toaplan2_tekipaki_state::c2map_r()
{
...
u8 toaplan2_tekipaki_state::tekipaki_cmdavailable_r()
{
...
void toaplan2_tekipaki_state::tekipaki_68k_mem(address_map &map)
{
	...
	map(0x180041, 0x180041).w(FUNC(toaplan2_tekipaki_state::coin_w));
...
void toaplan2_tekipaki_state::pipibibs_68k_mem(address_map &map)
{
	map(0x000000, 0x03ffff).rom();
	map(0x080000, 0x082fff).ram();
	map(0x0c0000, 0x0c0fff).ram().w(m_palette, FUNC(palette_device::write16)).share("palette");
	map(0x140000, 0x14000d).rw(m_vdp[0], FUNC(gp9001vdp_device::read), FUNC(gp9001vdp_device::write));
	map(0x190000, 0x190fff).rw(FUNC(toaplan2_tekipaki_state::shared_ram_r), FUNC(toaplan2_tekipaki_state::shared_ram_w)).umask16(0x00ff);
	map(0x19c01d, 0x19c01d).w(FUNC(toaplan2_tekipaki_state::coin_w));
...
void toaplan2_tekipaki_state::pipibibi_bootleg_68k_mem(address_map &map)
{
	map(0x000000, 0x03ffff).rom();
	map(0x080000, 0x082fff).ram();
...
//  map(0x13f000, 0x13f001).nopw();        // ???
	...
	map(0x190003, 0x190003).r(FUNC(toaplan2_tekipaki_state::shared_ram_r));  // Z80 ready ?
	map(0x190011, 0x190011).w(FUNC(toaplan2_tekipaki_state::shared_ram_w)); // Z80 task to perform
	map(0x19c01d, 0x19c01d).w(FUNC(toaplan2_tekipaki_state::coin_w));
	map(0x19c020, 0x19c021).portr("DSWA");
	map(0x19c024, 0x19c025).portr("DSWB");
	map(0x19c028, 0x19c029).portr("JMPR");
...
void toaplan2_tekipaki_state::cpu_space_pipibibsbl_map(address_map &map)
{
...
void toaplan2_tekipaki_state::pipibibs_sound_z80_mem(address_map &map)
{
...
void toaplan2_tekipaki_state::hd647180_io_map(address_map &map)
{
...
void toaplan2_tekipaki_state::tekipaki(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 10_MHz_XTAL);         // 10MHz Oscillator
	m_maincpu->set_addrmap(AS_PROGRAM, &toaplan2_tekipaki_state::tekipaki_68k_mem);
	m_maincpu->reset_cb().set(FUNC(toaplan2_tekipaki_state::toaplan2_reset));

	hd647180x_device &audiocpu(HD647180X(config, m_audiocpu, 10_MHz_XTAL));
	// 16k byte ROM and 512 byte RAM are internal
	audiocpu.set_addrmap(AS_IO, &toaplan2_tekipaki_state::hd647180_io_map);
	audiocpu.in_pa_callback().set(FUNC(toaplan2_tekipaki_state::tekipaki_cmdavailable_r));
	...
	m_screen->set_screen_update(FUNC(toaplan2_tekipaki_state::screen_update_toaplan2));
	m_screen->screen_vblank().set(FUNC(toaplan2_tekipaki_state::screen_vblank));
	...
	MCFG_VIDEO_START_OVERRIDE(toaplan2_tekipaki_state,toaplan2)
...
void toaplan2_tekipaki_state::pipibibs(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 10_MHz_XTAL);         // verified on PCB
	m_maincpu->set_addrmap(AS_PROGRAM, &toaplan2_tekipaki_state::pipibibs_68k_mem);
	m_maincpu->reset_cb().set(FUNC(toaplan2_tekipaki_state::toaplan2_reset));

	Z80(config, m_audiocpu, 27_MHz_XTAL/8);         // verified on PCB
	m_audiocpu->set_addrmap(AS_PROGRAM, &toaplan2_tekipaki_state::pipibibs_sound_z80_mem);
...
void toaplan2_tekipaki_state::pipibibsbl(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 12_MHz_XTAL); // ??? (position labeled "68000-12" but 10 MHz-rated parts used)
	m_maincpu->set_addrmap(AS_PROGRAM, &toaplan2_tekipaki_state::pipibibi_bootleg_68k_mem);
	m_maincpu->set_addrmap(m68000_base_device::AS_CPU_SPACE, &toaplan2_tekipaki_state::cpu_space_pipibibsbl_map);
	m_maincpu->reset_cb().set(FUNC(toaplan2_tekipaki_state::toaplan2_reset));
...
void toaplan2_tekipaki_state::init_pipibibsbl()
{
...
GAME( 1991, tekipaki,    0,        tekipaki,     tekipaki,   toaplan2_tekipaki_state, empty_init,    ROT0,   "Toaplan",         "Teki Paki",                 MACHINE_SUPPORTS_SAVE )
GAME( 1991, tekipakit,   tekipaki, tekipaki,     tekipaki,   toaplan2_tekipaki_state, empty_init,    ROT0,   "Toaplan",         "Teki Paki (location test)", MACHINE_SUPPORTS_SAVE )

GAME( 1991, pipibibs,    0,        pipibibs,     pipibibs,   toaplan2_tekipaki_state, empty_init,    ROT0,   "Toaplan",         "Pipi & Bibis / Whoopee!! (Z80 sound cpu, set 1)", MACHINE_SUPPORTS_SAVE )
GAME( 1991, pipibibsa,   pipibibs, pipibibs,     pipibibs,   toaplan2_tekipaki_state, empty_init,    ROT0,   "Toaplan",         "Pipi & Bibis / Whoopee!! (Z80 sound cpu, set 2)", MACHINE_SUPPORTS_SAVE )
GAME( 1991, pipibibsp,   pipibibs, pipibibs,     pipibibsp,  toaplan2_tekipaki_state, empty_init,    ROT0,   "Toaplan",         "Pipi & Bibis / Whoopee!! (prototype)",            MACHINE_SUPPORTS_SAVE )
GAME( 1991, whoopee,     pipibibs, tekipaki,     whoopee,    toaplan2_tekipaki_state, empty_init,    ROT0,   "Toaplan",         "Pipi & Bibis / Whoopee!! (Teki Paki hardware)",   MACHINE_SUPPORTS_SAVE ) // original Whoopee!! boards have a HD647180 instead of Z80

GAME( 1991, pipibibsbl,  pipibibs, pipibibsbl,   pipibibsbl, toaplan2_tekipaki_state, init_pipibibsbl, ROT0, "bootleg (Ryouta Kikaku)", "Pipi & Bibis / Whoopee!! (Ryouta Kikaku bootleg, encrypted)", MACHINE_SUPPORTS_SAVE )
GAME( 1991, pipibibsbl2, pipibibs, pipibibsbl,   pipibibsbl, toaplan2_tekipaki_state, empty_init,    ROT0,   "bootleg",                 "Pipi & Bibis / Whoopee!! (bootleg, decrypted)", MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE ) // different memory map, not scrambled
GAME( 1991, pipibibsbl3, pipibibs, pipibibsbl,   pipibibsbl, toaplan2_tekipaki_state, empty_init,    ROT0,   "bootleg (Ryouta Kikaku)", "Pipi & Bibis / Whoopee!! (Ryouta Kikaku bootleg, decrypted)", MACHINE_SUPPORTS_SAVE )
