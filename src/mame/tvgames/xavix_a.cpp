...
void xavix_sound_device::device_reset()
{
	...
	for (int v = 0; v < 16; v++)
	{
		...
		m_voice[v].env_active_left = true;
		m_voice[v].env_active_right = true;
...
void xavix_sound_device::sound_stream_update(sound_stream &stream)
{
	...
	while (num_samples-- > 0)
	{
		...
		// visit voices in order partial decap indicates
		for (int idx = 0; idx < 16; idx++)
		{
			...
			const int32_t envL = m_voice[v].env_vol_left;
			const int32_t envR = m_voice[v].env_vol_right;

			const int64_t base = int64_t(sample) * int64_t(gn);

			int64_t left = (base * int64_t(mvol)) / 255;
			left = (left * int64_t(envL)) / 255;
			total_l += left;

			int64_t right = (base * int64_t(mvol)) / 255;
			right = (right * int64_t(envR)) / 255;
			total_r += right;
...
void xavix_sound_device::enable_voice(int voice, bool update_only)
{
	...
	// Envelope registers
	const uint8_t  env_config = m_readregs_cb(base + 0x8); // [5:4]=vm, [3:0]=gain nibble
	...
	m_voice[voice].env_active_left = true;
	m_voice[voice].env_active_right = true;
	...
	else if (m_voice[voice].envmode == 1)
	{
		...
		const uint16_t startL = env_addr_left;
		const uint16_t startR = env_addr_right;
		...
		const uint8_t vL = fetch_env_byte_direct(voice, false, startL);
		const uint8_t vR = fetch_env_byte_direct(voice, true, startR);
		m_voice[voice].env_vol_left = vL;
		m_voice[voice].env_vol_right = vR;

		// Prime per-side phase counters to the *next* nibble entry
		m_voice[voice].envpositionleft = inc_low_nibble(startL);
		m_voice[voice].envpositionright = inc_low_nibble(startR);
		...
		m_writeregs_cb(regbase + 0x0A, uint8_t(startL)); // LA low
		m_writeregs_cb(regbase + 0x0C, uint8_t(startR)); // RA low
		return;
	}
	...
	else if (m_voice[voice].envmode == 2)
	{
		...
		auto step_side = [&](bool right) {
			uint32_t& pos = right ? m_voice[voice].envpositionright : m_voice[voice].envpositionleft;
			uint8_t& lvl = right ? m_voice[voice].env_vol_right : m_voice[voice].env_vol_left;
			const uint16_t addr = pos;
			const uint8_t  val = fetch_env_byte_direct(voice, right, addr);
			lvl = val;
			const bool adv = right ? (ra != 0) : (la != 0);
			if (adv) pos = uint16_t(addr + 1);
			};
...
uint8_t xavix_sound_device::sound_volume_r()          // 0x75F6 ('h6)
{
	return m_mix.mastervol;
}

void xavix_sound_device::sound_volume_w(uint8_t data) // 0x75F6 ('h6)
{
	set_mastervol(data);
}

uint8_t xavix_sound_device::sound_mixer_r()           // 0x75F9 ('h9)
{
...
void xavix_sound_device::sound_mixer_w(uint8_t data)  // 0x75F9 ('h9)
{
...
uint8_t xavix_sound_device::dac_control_r()           // 0x75FF ('hF)
{
	return (m_mix.dac << 7)
		| ((m_mix.gap & 0x03) << 5)
		| ((m_mix.lead & 0x07) << 2)
...
void xavix_sound_device::dac_control_w(uint8_t data)  // 0x75FF ('hF)
{
...
//-------------------------------------------------
//  timing helpers (tempo / cyclerate)
//-------------------------------------------------
...
uint8_t xavix_sound_device::fetch_env_byte(int voice, bool right, uint32_t idx)
{
	const xavix_voice& v = m_voice[voice];
	const uint16_t base = right ? v.env_rom_base_right : v.env_rom_base_left;
	const uint32_t addr = (uint32_t(v.envbank) << 16) | uint32_t((base + (idx & 0xffff)) & 0xffff);
	return m_readsamples_cb(addr);
}

uint8_t xavix_sound_device::fetch_env_byte_direct(int voice, bool right, uint16_t addr)
{
	...
	static bool first_seen[16][2] = { { false } }; // [voice][0=L,1=R]
...
void xavix_sound_device::step_envelope(int voice)
{
	...
	{
		auto step_side = [&](bool right) {
			if (right ? !v.env_active_right : !v.env_active_left) return;

			const uint16_t base = right ? v.env_rom_base_right : v.env_rom_base_left;
			uint32_t& phase = right ? v.envpositionright : v.envpositionleft;

			const uint8_t  ph = uint8_t(phase & 0x000F);
			const uint16_t addr = uint16_t((base & 0xFFF0) | ph);
			const uint8_t  lvl = fetch_env_byte_direct(voice, right, addr);
			if (right) v.env_vol_right = lvl; else v.env_vol_left = lvl;

			const int regbase = voice * 0x10;
			if (right) m_writeregs_cb(regbase + 0x0c, uint8_t(addr)); // RA low
			else       m_writeregs_cb(regbase + 0x0a, uint8_t(addr)); // LA low
	...
	if (v.envmode == 2)
	{
		auto step_side = [&](bool right) {
			if (right ? !v.env_active_right : !v.env_active_left) return;
			uint32_t& pos = right ? v.envpositionright : v.envpositionleft;
			const uint8_t val = fetch_env_byte_direct(voice, right, pos);
			if (right) v.env_vol_right = val; else v.env_vol_left = val;
			pos = uint16_t(pos + 1);
		};
