...
inline void xavix_sound_device::step_side1(bool right, int voice, const uint8_t la, const uint8_t ra)
{
...
void xavix_sound_device::enable_voice(int voice, bool update_only)
{
	...
	else if (m_voice[voice].envmode == 2)
	{
		...
		step_side1(false, voice, la, ra);
		step_side1(true, voice, la, ra);
		return;
	}
...
void xavix_sound_device::sound_mixer_w(uint8_t data) // 0x75f9
{
...
uint8_t xavix_sound_device::dac_control_r() // 0x75ff
{
	return (m_mix.dac << 7)
		| ((m_mix.gap & 0x03) << 5)
		| ((m_mix.lead & 0x07) << 2)
...
void xavix_sound_device::dac_control_w(uint8_t data) // 0x75ff
{
...
inline void xavix_sound_device::step_side_env_vm1(bool right, xavix_voice v, int voice)
{
	if (right ? !v.env_active_right : !v.env_active_left)
		return;
...
inline void xavix_sound_device::step_side_env_vm2(bool right, xavix_voice v, int voice)
{
	if (right ? !v.env_active_right : !v.env_active_left)
		return;
...
void xavix_sound_device::step_envelope(int voice)
{
	...
	if (v.envmode == 1)
	{
		step_side_env_vm1(false, v, voice);
		step_side_env_vm1(true, v, voice);
	...
	if (v.envmode == 2)
	{
		step_side_env_vm2(false, v, voice);
		step_side_env_vm2(true, v, voice);
