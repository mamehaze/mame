...
{
	logerror("%s:generalplus_gpl162xx_b_base::esp_ctrl_w %04x\n", machine().describe_context(), data);
	m_esp_ctrl = data;
}

u16 generalplus_gpl162xx_b_base::esp_ctrl_r()
{
	logerror("%s:generalplus_gpl162xx_b_base::esp_ctrl_r\n", machine().describe_context());
	return m_esp_ctrl;
}
...
{
	logerror("%s:generalplus_gpl162xx_b_base::byteswap_w %04x\n", machine().describe_context(), data);
	m_byteswap = data;
}

u16 generalplus_gpl162xx_b_base::byteswap_r()
{
	logerror("%s:generalplus_gpl162xx_b_base::byteswap_r\n", machine().describe_context());
...
void generalplus_gpl16218b_device::device_add_mconfig(machine_config & config)
{
	...
	// supports SPI? (unlike GPL16220A?) (unless there's internal ROM no booting from it though?)
}
