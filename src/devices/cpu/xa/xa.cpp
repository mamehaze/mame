...
// SFR names
const xa_cpu_device::mem_info xa_cpu_device::default_names[] = {
...
void xa_cpu_device::add_names(const mem_info *info)
{
...
std::string xa_cpu_device::get_bittext(int bit)
{
	int position = bit & 7;
...
std::string xa_cpu_device::get_directtext(int direct)
{
	if (direct < 0x400)
	{
...
void xa_cpu_device::d_illegal(XA_EXECUTE_PARAMS)
{
	fatalerror( "illegal");
}
...
void xa_cpu_device::handle_alu_type0(XA_EXECUTE_PARAMS, int alu_op)
{
	...
	case 0x02:
	{
		...
		else
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_indrd_rs(rd, rs); break;
			...
			else
			{
				...
				case 0x3: subc_byte_indrd_rs(rd, rs); break;
	...
	case 0x03:
	{
		...
		if (!optype)
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_rd_indrsinc(rd, rs); break;
			...
			else
			{
				...
				case 0x3: subc_byte_rd_indrsinc(rd, rs); break;
		...
		else
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_indrdinc_rs(rd, rs); break;
			...
			else
			{
				...
				case 0x3: subc_byte_indrdinc_rs(rd, rs); break;
	...
	case 0x04:
	{
		...
		if (!optype)
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_rd_rsoff8(rd, rs, offset8); break;
			...
			else
			{
				...
				case 0x3: subc_byte_rd_rsoff8(rd, rs, offset8); break;
		...
		else
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_rdoff8_rs(rd, offset8, rs); break;
			...
			else
			{
				...
				case 0x3: subc_byte_rdoff8_rs(rd, offset8, rs); break;
	...
	case 0x05:
	{
		...
		if (!optype)
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_rd_rsoff16(rd, rs, offset16); break;
			...
			else
			{
				...
				case 0x3: subc_byte_rd_rsoff16(rd, rs, offset16); break;
		...
		else
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_rdoff16_rs(rd, offset16, rs); break;
			...
			else
			{
				...
				case 0x3: subc_byte_rdoff16_rs(rd, offset16, rs); break;
	...
	case 0x06:
	{
		...
		if (!optype)
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_rd_direct(rd, direct); break;
			...
			else
			{
				...
				case 0x3: subc_byte_rd_direct(rd, direct); break;
		...
		else
		{
			...
			if (size)
			{
				...
				case 0x3: subc_word_direct_rs(direct, rs); break;
			...
			else
			{
				...
				case 0x3: subc_byte_direct_rs(direct, rs); break;
...
void xa_cpu_device::handle_alu_type1(XA_EXECUTE_PARAMS, uint8_t op2)
{
	...
	case 0x01: // ALUOP.b Rd, data8
	{
		...
		default: fatalerror( "UNK_ALUOP.b %s, #$%02x", m_regnames8[rd], data8 );
		}
		return;
	}
	...
	case 0x02: // ALUOP.b [Rd], data8
	{
		...
		default: fatalerror( "UNK_ALUOP.b [%s], #$%02x", m_regnames16[rd], data8 );
		}
		return;
	}
	...
	case 0x04: // ALUOP.b [Rd+offs8], data8
	{
		...
		default: fatalerror( "UNK_ALUOP.b [%s+#$%02x], #$%02x", m_regnames16[rd], offset8, data8 );
		}
		return;
	}
	...
	case 0x05: // ALUOP.b [Rd+offs16], data8
	{
		...
		default: fatalerror( "UNK_ALUOP.b [%s+#$%04x], #$%02d", m_regnames16[rd], offset16, data8 );
		}
		return;
	}
	...
	case 0x09: // ALUOP.w Rd, data16
	{
		...
		case 0x3: subc_word_rd_data16(rd, data16); break;
		...
		default: fatalerror("UNK_ALUOP.w %s, #$%04x (RD, DATA16)", m_regnames16[rd], data16); break;
		}
		return;
	}
	...
	case 0x0a: // ALUOP.w [Rd], data16
	{
		...
		default: fatalerror( "UNK_ALUOP.w [%s], #$%04x", m_regnames16[rd], data16 );
		}
		return;
	}
	...
	case 0x0c: // ALPOP.w [Rd+offs8], data16
	{
		...
		default: fatalerror( "UNK_ALUOP.w [%s+#$%02x], #$%04x", m_regnames16[rd], offset8, data16 );
		}
		return;
	}
	...
	case 0x0d: // ALUOP.w [Rd+offs16], data16
	{
		...
		default: fatalerror( "UNK_ALUOP.w [%s+#$%04x], #$%04x", m_regnames16[rd], offset16, data16  );
		}
		return;
	}
	...
	case 0x0e: // ALUOP.w DIRECT, data16
	{
		...
		default: fatalerror( "UNK_ALUOP.w %s, #$%04x", get_directtext(direct), data16 );
		}
		return;
	}
	}
...
void xa_cpu_device::handle_adds_movs(XA_EXECUTE_PARAMS, int which)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
...
void xa_cpu_device::d_bitgroup(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 op3 = m_program->read_byte(m_pc++);
	...
	default:   fatalerror( "illegal bit op %s", get_bittext(bit) );	break;
	}
	return;
}
...
*/
void xa_cpu_device::d_add(XA_EXECUTE_PARAMS)
{
...
void xa_cpu_device::d_addc(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_sub(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_subb(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_lea_offset16(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 op3 = m_program->read_byte(m_pc++);
	const u8 op4 = m_program->read_byte(m_pc++);
...
*/
void xa_cpu_device::d_cmp(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_xch_type1(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
	const char** regnames = size ? m_regnames16 : m_regnames8;
...
*/
void xa_cpu_device::d_xch_type2(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
	const char** regnames = size ? m_regnames16 : m_regnames8;
...
*/
void xa_cpu_device::d_or(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_xor(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_movc_rd_rsinc(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
...
*/
void xa_cpu_device::d_mov(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_g9_subgroup(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
	const char** regnames = size ? m_regnames16 : m_regnames8;
...
*/
void xa_cpu_device::d_alu(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
...
*/
void xa_cpu_device::d_jb_mov_subgroup(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 op3 = m_program->read_byte(m_pc++);
	const u8 op4 = m_program->read_byte(m_pc++);
...
*/
void xa_cpu_device::d_movdir(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 op3 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
...
*/
void xa_cpu_device::d_adds(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_movx_subgroup(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
	const char** regnames = size ? m_regnames16 : m_regnames8;
...
*/
void xa_cpu_device::d_movs(XA_EXECUTE_PARAMS)
{
...
*/
void xa_cpu_device::d_rrc(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	int size = op & 0x08;
	const char** regnames = size ? m_regnames16 : m_regnames8;
...
*/
void xa_cpu_device::d_rl(XA_EXECUTE_PARAMS)
{
	int size = op & 0x08;
	const u8 op2 = m_program->read_byte(m_pc++);
	const char** regnames = size ? m_regnames16 : m_regnames8;
...
*/
void xa_cpu_device::d_rlc(XA_EXECUTE_PARAMS)
{
	int size = op & 0x08;
	const u8 op2 = m_program->read_byte(m_pc++);
	const char** regnames = size ? m_regnames16 : m_regnames8;
...
*/
void xa_cpu_device::d_djnz_cjne(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 op3 = m_program->read_byte(m_pc++);
	const u8 op4 = m_program->read_byte(m_pc++);
...
*/
void xa_cpu_device::d_mulu_b(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xf0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_divu_b(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xf0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_mulu_w(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xf0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_divu_w(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xf0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_mul_w(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xf0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_div_w(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xf0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_divu_d(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xe0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_div_d(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 rd = (op2 & 0xe0) >> 4;
	const u8 rs = (op2 & 0x0f);
...
*/
void xa_cpu_device::d_cjne_d8(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 op3 = m_program->read_byte(m_pc++);
	const u8 op4 = m_program->read_byte(m_pc++);
...
*/
void xa_cpu_device::d_cjne_d16(XA_EXECUTE_PARAMS)
{
	const u8 op2 = m_program->read_byte(m_pc++);
	const u8 op3 = m_program->read_byte(m_pc++);
	const u8 op4 = m_program->read_byte(m_pc++);
...
*/
void xa_cpu_device::d_bkpt(XA_EXECUTE_PARAMS)
{
	fatalerror( "BKPT");
}
